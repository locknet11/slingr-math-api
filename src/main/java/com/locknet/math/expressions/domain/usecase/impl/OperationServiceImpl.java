...
public class OperationServiceImpl implements OperationService{
	...
	public Number parseAndCalculate(String expression, Optional<Integer> precision) throws ExpressionFormatException {
		 ...
		 * then if present, it will use BigDecimal class to apply the truncation
		 * if not, it will just return the original number
		 */
		...
		if(precision.isPresent()) {
			return BigDecimal.valueOf(result)
					.setScale(precision.get(), BigDecimal.ROUND_DOWN)
					.doubleValue();
		}else {
			return result;
		}
	...
	/*
	 * This method validates the expression correctness, if it's malformed it will thrown
	 * a ExpressionFormatException
	 */
